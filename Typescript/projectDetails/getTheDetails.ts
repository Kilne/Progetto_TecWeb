function populate(project_details: data_orm) {
    for (let projectDetailsKey in project_details) {

        switch (projectDetailsKey) {
            case "due_date":
                let date: Date = new Date(project_details[projectDetailsKey]);
                let [year, month, day] = [date.getFullYear(), date.getMonth() + 1, date.getDate()];
                document.getElementById(
                    projectDetailsKey).lastElementChild.innerHTML = `${day}/${month}/${year}`;
                break;
            case "perc_compl":
                (document.getElementById("progress").setAttribute(
                    "value", String(project_details[projectDetailsKey])));
            ...
            default:
                document.getElementById(
                    projectDetailsKey).lastElementChild.innerHTML = project_details[projectDetailsKey];
                break;
        ...
        (
            ...
            (data) => {
                if (data["data"] !== false) {
                    ...
                    let project_details: data_orm = data["data"];
