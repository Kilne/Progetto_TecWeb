function radio_and_buttons_enable() {
    document.getElementById("buttons_options_confirm").setAttribute(
        "style", "visibility: visible");
    document.getElementById("buttons_options_refuse").setAttribute(
        "style", "visibility: visible");
    ...
    for (let elementsByClassNameElement of document.getElementsByTagName("input")) {
        if (elementsByClassNameElement.id === "project_list_column_element_box_checkbox") {
            (elementsByClassNameElement as HTMLElement).style.visibility = "visible"
        }
    }
    ...
    for (let elementsByTagNameElement of document.getElementsByTagName("label")) {
        if (elementsByTagNameElement.id === "project_list_column_element_box_label") {
            (elementsByTagNameElement as HTMLElement).style.visibility = "visible"
        }
    }
...
function radio_and_buttons_disable() {
    ...
    for (let elementsByClassNameElement of document.getElementsByTagName("input")) {
        if (elementsByClassNameElement.id === "project_list_column_element_box_checkbox") {
            (elementsByClassNameElement as HTMLElement).style.visibility = "hidden"
        }
    }

    for (let elementsByTagNameElement of document.getElementsByTagName("label")) {
        if (elementsByTagNameElement.id === "project_list_column_element_box_label") {
            (elementsByTagNameElement as HTMLElement).style.visibility = "hidden"
        }
    }
...
document.addEventListener("click", function (event) {
    ...
    if (target.closest("#buttons_options_confirm")) {
        ...
        let collected_checkboxes: HTMLInputElement[] = [];

        for (let elements of document.getElementsByTagName("input")) {

            if (elements.id === "project_list_column_element_box_checkbox" && (elements as HTMLInputElement).checked) {

                collected_checkboxes.push(elements as HTMLInputElement);

                let next_cyle : Element = elements
                ...
                while(next_cyle.nextElementSibling != null){
                    ...
                    if(next_cyle.id === "project_list_column_element_box_internal_unique_id"){
                        ...
                        project_id.push((next_cyle as HTMLElement).innerHTML);
