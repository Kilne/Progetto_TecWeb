...
function get_the_existing_project() {
    let date_arranger = document.getElementById("due_date").lastElementChild.innerHTML
        .split("/");
    return {
        ...
        num_steps_completed: parseInt(document.getElementById("num_steps_completed").lastElementChild.innerHTML),
...
function get_the_new_project() {
    let date_arranger = document.getElementById("due_date_edit").value.split("-");
    let new_name = document.getElementById("p_name_edit").value;
    let new_obj = document.getElementById("obj_edit").value;
    let new_description = document.getElementById("description_edit").value;
    let new_due_date = new Date(date_arranger[0] + "/" + date_arranger[1] + "/" + date_arranger[2]);
    let new_goal = parseInt(document.getElementById("goal_edit").value);
    let new_num_steps_total = parseInt(document.getElementById("num_steps_total_edit").value);
    let existing_project = get_the_existing_project();
    ...
    if (wrong_input) {
        ...
        return {
            ...
            single_step_value: new_goal / Math.floor((new_due_date.getTime() - new Date().getTime()) / (86400 * 1000)),
            time_left: Math.floor((new_due_date.getTime() - new Date().getTime()) / (86400 * 1000)),
        };
    }
}
function form_filler() {
    ...
    document.getElementById("p_name_edit").value = existing_project.p_name;
    document.getElementById("obj_edit").value = existing_project.obj;
    document.getElementById("description_edit").value = existing_project.description;
    document.getElementById("due_date_edit").value = date_correction.join("-");
    document.getElementById("goal_edit").value = existing_project.goal.toString();
    document.getElementById("num_steps_total_edit").value =
...
document.addEventListener("click", function (event) {
    ...
    if (target.closest("#submit_changes")) {
        let new_project = get_the_new_project();
